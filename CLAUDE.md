...
> **このリポジトリ（FREE WAN 大阪北区店）の目標要件は `docs/osaka-kitaku/要件定義書.md`（R1〜R9）。**
> 上の4種類はその一部。画面・API に出す R番号は要件書に合わせてある（休眠フォローは R10 として採用、R5 のみ未実装で欠番）。
> ジョブの**保存キー**（`src/reminders.js` の key。message_logs 等に残る）だけは実装順の歴史的な名前のままで、
> R番号との対応は `src/reminders.js` の REMINDER_JOBS が正。要件と実装の対応表は `docs/handover.md` の2章。
