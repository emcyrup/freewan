...
公開リポジトリなので、サーバー側は HTTPS で clone すればよく、鍵の登録は要らない（下の手順1）。
...
> **DB 名の発行待ちの間**は、ここで止まる（マイグレーションが DB に繋がらないため）。
> 待っている間に進められるのは次の3つ。DB が来たら上の1行に戻ればよい。
>
> ```bash
> npm ci --omit=dev     # 依存の導入。DB は要らない
> ```
>
> - `.env` の DB 以外の値を埋める（LINE のトークンなどは現行サーバーからコピー）
> - 手順4 の PM2 の導入と PATH 通し（起動はまだしない）
